...
df = pd.read_csv("export/annual-tilt38/raw/Annualall.csv", parse_dates=True, index_col='time')
...
hist_weights, _, _ = np.histogram2d(df["azimuth"], df["zenith"], bins=(abins, rbins), weights=df[quantity])
hist_weights_kwh, _, _ = np.histogram2d(df["azimuth"], df["zenith"], bins=(abins, rbins), weights=df[quantity]/60000)
...
plot_polar_heatmap(hist_weights_kwh, cbar_label="$kWh$", pic_path=f"{folder}/kwh")
