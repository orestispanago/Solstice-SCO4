...
def read(fname):
    ...
    df_out["efficiency"] = df.loc[
        df[0] == "entity_all.absorber", [23]
    ].values  # Overall effficiency, add [23,24] for error
    for key in columns.keys():
        df_out[key] = (
            df[0].iloc[df_out.index + columns.get(key)].astype("float").values
        )
    return df_out
...
annual_df1 = pd.read_csv(
    annual.csv_path.split(".")[0] + ".csv", index_col="time", parse_dates=True
)
